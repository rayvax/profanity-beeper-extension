...
import {
  CensorAudioEffect,
  scheduleCensorEffect,
  type CensorAudioEffectValue,
} from './censor-effect';
...
export function createBeepCensorExecutor(
  ...
  const reschedule = () => {
    ...
    scheduledRanges.forEach((scheduledRange) =>
      scheduleRange(currentGraph, scheduledRange, effect),
    );
  };
  ...
  return {
    ...
    async arm() {
      ...
      if (!media.paused) {
        scheduledRanges.forEach((scheduledRange) =>
          scheduleRange(playbackGraph, scheduledRange, effect),
        );
      }
    },
    ...
    async execute(range) {
      ...
      return new Promise<void>((resolve, reject) => {
        ...
        if (graph && !media.paused) {
          scheduleRange(graph, scheduledRange, effect);
        } else if (!graph) {
          void ensureGraph(media).then(
            (playbackGraph) => {
              if (graph !== playbackGraph || !scheduledRanges.has(scheduledRange)) return;
              listenTo(media);
              if (!media.paused) scheduleRange(playbackGraph, scheduledRange, effect);
            },
...
function scheduleRange(
  ...
  scheduledRange.timer = setTimeout(() => {
    startRange(graph, scheduledRange, effect);
  }, delayMs);
}
...
function startRange(
  ...
  try {
    ...
    const merged = graph.hasWindow && startTime <= graph.mutedUntil + 0.05;
    graph.mutedUntil = Math.max(graph.mutedUntil, endTime);
    graph.hasWindow = true;
    graph.gain.gain.cancelScheduledValues(startTime);
    if (merged) {
      graph.gain.gain.setValueAtTime(0, startTime);
    } else {
      graph.gain.gain.setValueAtTime(1, startTime);
      graph.gain.gain.linearRampToValueAtTime(0, startTime + 0.01);
    }
    graph.gain.gain.setValueAtTime(0, graph.mutedUntil);
    graph.gain.gain.linearRampToValueAtTime(1, graph.mutedUntil + 0.01);
  } catch (error) {
